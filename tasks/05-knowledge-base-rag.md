...
**Build order:** TRD §17, step 5. **Depends on:** Phase 4 (Memory layer —
...
- Ingestion pipeline per TRD §13: sources → sync trigger → ingestion queue
...
- Notion sync: API webhooks (shipped 2026, signature-verified) as the
  primary trigger; daily poll from TRD §10.3 as the safety net. Every
...
- `search_knowledge_base` tool per the contract in TRD §13: input
...
- Additional sources (Gmail/Drive/Calendar/Spotify/YouTube content) — not
  planned for KB ingestion per TRD's scope (§1); those are live-integration
  reads, not indexed knowledge.

## References

TRD §11.1 (Notion webhooks/rate limits), §12.1/§12.2 (kb_documents, vector
DB schema), §13 (ingestion pipeline + retrieval tool contract), §17 step 5.
